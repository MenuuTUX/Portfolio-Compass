      ...
      try {
        ...
        if (topGainers.length > 0) {
          const res = await fetch(
            `/api/etfs/search?tickers=${topGainers.join(",")}&includeHistory=true`,
          );
        ...
        if (topLosers.length > 0) {
          const res = await fetch(
            `/api/etfs/search?tickers=${topLosers.join(",")}&includeHistory=true`,
          );
          const raw = await res.json();
          try {
        ...
        ); // Ensure sorted by % ascending (most negative first)
