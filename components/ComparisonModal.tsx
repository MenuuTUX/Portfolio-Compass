...
import { X, Search, ChevronRight, TrendingUp, DollarSign, Activity, Scale, Trophy, Minus, Layers } from 'lucide-react';
  ...
  return (
    <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-4 py-3 border-b border-white/5 last:border-0 hover:bg-white/5 transition-colors px-4 rounded-lg">
      ...
      <div className="flex flex-col items-center justify-center min-w-[100px]">
        <span className="text-xs text-neutral-500 uppercase font-semibold tracking-wider text-center">{label}</span>
        {better && better !== 'Equal' && (
          <div className={cn(
         ...
         {better === 'Equal' && <Minus className="w-3 h-3 mt-1 text-neutral-600" />}
      </div>
...
export default function ComparisonModal({ baseAsset, isOpen, onClose }: ComparisonModalProps) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      setIsSearching(true);
      try {
        const typeFilter = baseAsset.assetType ? `&type=${baseAsset.assetType}` : '';
        const res = await fetch(`/api/etfs/search?query=${searchQuery}${typeFilter}`);
        if (res.ok) {
          const data = await res.json();
          // Filter out the base asset
          setSearchResults(data.filter((item: ETF) => item.ticker !== baseAsset.ticker));
        }
      } catch (err) {
  ...
  const handleSelectAsset = async (asset: ETF) => {
    ...
    try {
      ...
      const res = await fetch(`/api/etfs/search?query=${asset.ticker}&full=true`);
      if (res.ok) {
        const data = await res.json();
        const fullDetails = data.find((item: ETF) => item.ticker === asset.ticker);
        if (fullDetails) {
  ...
  const metrics = useMemo(() => {
    ...
    const addMetric = (label: string, key: keyof ETF | string, rule: 'high' | 'low' | 'none', formatter: (v: number) => string, deepKey?: string) => {
    ...
    addMetric('Change', 'changePercent', 'high', (v) => `${v > 0 ? '+' : ''}${v.toFixed(2)}%`);
    addMetric('Market Cap', 'marketCap', 'high', (v) => (v/1e9).toFixed(2) + 'B');
    ...
    if (isStock) {
        ...
        addMetric('Revenue', 'revenue', 'high', (v) => (v/1e9).toFixed(2) + 'B');
        addMetric('Div Yield', 'dividendYield', 'high', (v) => `${v.toFixed(2)}%`);
    } else {
        // ETF Specific
        addMetric('Expense Ratio', 'metrics', 'low', (v) => `${v.toFixed(2)}%`, 'mer');
        addMetric('Yield', 'metrics', 'high', (v) => `${v.toFixed(2)}%`, 'yield');
        addMetric('Holdings', 'holdingsCount', 'high', (v) => Math.round(v).toLocaleString()); // More holdings = more diversification? Subjective but let's say high wins
    }
  ...
  const chartData = useMemo(() => {
    ...
    const historyA = baseAsset.history.filter(h => !h.interval || h.interval === '1d' || h.interval === '1wk');
    const historyB = compareAsset.history.filter(h => !h.interval || h.interval === '1d' || h.interval === '1wk');

    if (historyA.length === 0 || historyB.length === 0) return [];

    // Sort by date
    const sortedA = [...historyA].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
    const sortedB = [...historyB].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
    ...
    const filteredA = sortedA.filter(h => new Date(h.date).getTime() >= commonStart);
    const filteredB = sortedB.filter(h => new Date(h.date).getTime() >= commonStart);
    ...
    const dataPoints = filteredA.map(ptA => {
        const dateStr = ptA.date;
        const dateObj = new Date(dateStr);

        // Find closest date in B (within 3 days tolerance)
        const ptB = filteredB.find(p => Math.abs(new Date(p.date).getTime() - dateObj.getTime()) < 3 * 24 * 60 * 60 * 1000);
            ...
            <div className="grid grid-cols-[1fr_auto_1fr] gap-4 mb-8">
                ...
                <div className="flex flex-col items-center p-4 bg-emerald-900/10 border border-emerald-500/20 rounded-xl">
                    <div className="w-12 h-12 mb-3">
                         {getAssetIconUrl(baseAsset.ticker, baseAsset.name, baseAsset.assetType) && (
                            <img src={getAssetIconUrl(baseAsset.ticker, baseAsset.name, baseAsset.assetType)!} alt="A" className="w-full h-full object-contain" />
                         )}
                    </div>
                    <div className="text-2xl font-bold text-white">{baseAsset.ticker}</div>
                    <div className="text-sm text-neutral-400 text-center line-clamp-1">{baseAsset.name}</div>
                    <div className={cn("mt-2 font-mono", baseAsset.changePercent >= 0 ? "text-emerald-400" : "text-rose-400")}>
                        {baseAsset.changePercent >= 0 ? '+' : ''}{baseAsset.changePercent.toFixed(2)}%
                    </div>
                </div>

                {/* VS Divider */}
                <div className="flex flex-col items-center justify-center">
                    <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center border border-white/10 font-bold text-neutral-500 text-xs">VS</div>
                </div>
                ...
                {compareAsset ? (
                     <div className="relative flex flex-col items-center p-4 bg-blue-900/10 border border-blue-500/20 rounded-xl group">
                        ...
                        <div className="w-12 h-12 mb-3">
                            {getAssetIconUrl(compareAsset.ticker, compareAsset.name, compareAsset.assetType) && (
                                <img src={getAssetIconUrl(compareAsset.ticker, compareAsset.name, compareAsset.assetType)!} alt="B" className="w-full h-full object-contain" />
                            )}
                        </div>
                        <div className="text-2xl font-bold text-white">{compareAsset.ticker}</div>
                        <div className="text-sm text-neutral-400 text-center line-clamp-1">{compareAsset.name}</div>
                        <div className={cn("mt-2 font-mono", compareAsset.changePercent >= 0 ? "text-emerald-400" : "text-rose-400")}>
                            {compareAsset.changePercent >= 0 ? '+' : ''}{compareAsset.changePercent.toFixed(2)}%
                        </div>
                     </div>
                ) : (
                                        ...
                                        searchResults.map(item => (
                                            ...
                                            >
                                                <div>
                                                    ...
                                                    <div className="text-xs text-neutral-400 truncate w-40">{item.name}</div>
                                                </div>
                                                <ChevronRight className="w-4 h-4 text-neutral-600 group-hover:text-emerald-400" />
                                            </button>
                                        ))
                                    ) : (
                                        <div className="p-4 text-center text-xs text-neutral-500">No matching assets found</div>
                                    )}
                                </div>
                            )}
                        </div>
                        <div className="mt-4 text-xs text-neutral-500">Select a {baseAsset.assetType === 'STOCK' ? 'Stock' : 'ETF'} to compare</div>
                    </div>
                )}
            </div>
                ...
                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="bg-white/5 rounded-2xl p-6 border border-white/5">
                        <div className="flex items-center gap-2 mb-6">
                            ...
                            <h3 className="text-lg font-bold text-white">Performance Comparison (Normalized)</h3>
                        </div>
                    ...
                    <div className="bg-white/5 rounded-2xl p-6 border border-white/5">
                         <div className="flex items-center gap-2 mb-6">
                            ...
                            <h3 className="text-lg font-bold text-white">Head-to-Head Metrics</h3>
                        </div>
                    ...
                    {baseAsset.assetType === 'ETF' && compareAsset.assetType === 'ETF' && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {/* Asset A Sector */}
                            <div className="bg-white/5 rounded-2xl p-6 border border-white/5 flex flex-col items-center">
                                <h4 className="text-sm font-bold text-white mb-4">{baseAsset.ticker} Allocation</h4>
                                <div className="w-full h-[200px] flex justify-center">
                                    <SectorPieChart data={baseAsset.sectors ? Object.entries(baseAsset.sectors).map(([k, v]) => ({ name: formatSectorName(k), value: v * 100 })).sort((a,b) => b.value - a.value) : []} />
                                </div>
                            </div>
                             {/* Asset B Sector */}
                            <div className="bg-white/5 rounded-2xl p-6 border border-white/5 flex flex-col items-center">
                                <h4 className="text-sm font-bold text-white mb-4">{compareAsset.ticker} Allocation</h4>
                                <div className="w-full h-[200px] flex justify-center">
                                    <SectorPieChart data={compareAsset.sectors ? Object.entries(compareAsset.sectors).map(([k, v]) => ({ name: formatSectorName(k), value: v * 100 })).sort((a,b) => b.value - a.value) : []} />
                                </div>
                            </div>
                        </div>
                    )}
