...
export const useBatchAddPortfolio = () => {
  ...
  return useMutation({
    mutationFn: async (payload: BatchAddPayload | BatchAddItem[]) => {
      ...
      const response = await fetch(`/api/etfs/search?tickers=${tickers}&includeHistory=true`);
      if (!response.ok) {
    ...
    onSuccess: (data) => {
        ...
        queryClient.setQueryData<Portfolio>(['portfolio'], (oldPortfolio) => {
             ...
             return updatedPortfolio.map(item => {
                 const richData = stockMap.get(item.ticker.toUpperCase()) || oldMap.get(item.ticker.toUpperCase());
